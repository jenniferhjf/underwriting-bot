这是一个非常棒的功能升级！你提供的参考代码（v2.8.2）使用了非常完善的“工作区（Workspace）”概念。

针对你之前运行的 **v4.1.0 (PIL版本)** 代码，我提取了文件上传的核心逻辑，并将其无缝集成进去。

**主要修改点：**

1. **重构代码结构**：将原本写在循环里的“文档处理逻辑”提取为独立的函数 `process_document`，这样既可以处理预设文件，也可以处理上传文件。
2. **新增侧边栏上传区**：在 Sidebar 增加了 `st.file_uploader`。
3. **实时处理**：上传文件后，点击“Process”会立即保存文件、运行 OCR、生成向量并存入数据库，无需重启。

以下是完整的 **v4.2.0** 代码，你可以直接复制替换：
